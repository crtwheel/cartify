...
# Copyright 2023-present Cartify contributors.
...
if ! is_root && [ "${override_root:-0}" -eq 0 ]; then
    ...
    echo "If you hadn't intended to do this, please execute the script without root access to avoid problems with Cartify"
...
# download uri
releases_uri=https://github.com/crtwheel/cartify/releases
if [ -z "$tag" ]; then
...
download_uri=$releases_uri/download/v$tag/Cartify-$tag-$target.tar.gz

# locations
Cartify_install="$HOME/.Cartify"
exe="$Cartify_install/Cartify"
tar="$Cartify_install/Cartify.tar.gz"

# installing
[ ! -d "$Cartify_install" ] && log "CREATING $Cartify_install" && mkdir -p "$Cartify_install"

log "DOWNLOADING $download_uri"
curl --fail --location --progress-bar --output "$tar" "$download_uri"

log "EXTRACTING $tar"
tar xzf "$tar" -C "$Cartify_install"
...
export Cartify_INSTALL="$Cartify_install"
export PATH="\$PATH:$Cartify_install"
EOINFO
}
...
check() {
    path="export PATH=\$PATH:$Cartify_install"
    shellrc=$HOME/$1
    ...
    if [ -f "$shellrc" ]; then
        if ! grep -q "$Cartify_install" "$shellrc"; then
            log "APPENDING $Cartify_install to PATH in $shellrc"
            if ! endswith_newline "$shellrc"; then
                echo >> "$shellrc"
            fi
            echo "${2:-$path}" >> "$shellrc"
            export PATH="$Cartify_install:$PATH"
        else
            log "Cartify path already set in $shellrc, continuing..."
        fi
    else
        notfound
    fi
...
case $SHELL in
    ...
    *bash)
    ...
    ;;
    *fish) check ".config/fish/config.fish" "fish_add_path $Cartify_install" ;;
    *) notfound ;;
esac

echo
log "Cartify v$tag was installed successfully to $Cartify_install"
log "Run 'Cartify --help' to get started"
