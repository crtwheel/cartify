...
let dismissed;
try {
	dismissed = JSON.parse(Cartify.LocalStorage.get("new-releases:dismissed"));
...
class Grid extends react.Component {
	...
	removeCards(id, type) {
		switch (type) {
			case "reset":
				Cartify.showNotification("Reset dismissed releases");
				dismissed = [];
				break;
			case "undo":
				if (!dismissed[0]) Cartify.showNotification("Nothing to undo", true);
				else Cartify.showNotification("Undone dismissal");
		...
		Cartify.LocalStorage.set("new-releases:dismissed", JSON.stringify(dismissed));
	...
	async componentDidMount() {
		...
		this.configButton = new Cartify.Menu.Item(
			"New Releases config",
			false,
			openConfig,
	...
	render() {
		...
		const version = Cartify.Platform.version.split(".").map((i) => Number.parseInt(i));
		...
		return react.createElement(
			...
			react.createElement(
				...
				react.createElement("h1", null, Cartify.Locale.get("new_releases")),
				react.createElement(
					"div",
					{
					...
					react.createElement(ButtonText, {
						text: Cartify.Locale.get("playlist.extender.refresh"),
...
async function getArtistList() {
	...
	const artists = await Cartify.Platform.LibraryAPI.getContents(config);
	count(true);
	return artists.items ?? [];
}

async function getArtistEverything(artist) {
	const { data, errors } = await Cartify.GraphQL.Request(
		{
	...
	const items = [];
	const types = [
		[CONFIG.album, releases.filter((r) => r.type === "ALBUM"), Cartify.Locale.get("album")],
		...
		// [CONFIG["appears-on"], releases.appears_on?.releases, Cartify.Locale.get("artist.appears-on")],
		[CONFIG.compilations, releases.filter((r) => r.type === "COMPILATION"), Cartify.Locale.get("compilation")],
		[
			CONFIG["single-ep"],
			releases.filter((r) => r.type === "SINGLE" || r.type === "EP"),
			`${Cartify.Locale.get("single")}/${Cartify.Locale.get("ep")}`,
		],
	];
...
async function getPodcastList() {
	const body = await Cartify.Platform.LibraryAPI.getShows({ limit: 50000 });
	return body.items ?? [];
}

async function getPodcastRelease(uri) {
	const body = await Cartify.Platform.ShowAPI.getContents(uri, { limit: 50000 });
	return body.items;
}
...
async function fetchTracks() {
	...
	Cartify.showNotification(`Fetching releases from ${artistList.length} artists`);
...
async function fetchPodcasts() {
	const items = [];
	const itemTypeStr = Cartify.Locale.get("card.tag.episode");
