function DraggableComponent({ uri, title, children }) {
	const dragHandler = Cartify.ReactHook.DragHandler?.([uri], title);
	return dragHandler
...
class Card extends react.Component {
	constructor(props) {
		...
		const uriType = Cartify.URI.fromString(this.uri)?.type;
		switch (uriType) {
			case Cartify.URI.Type.ALBUM:
			case Cartify.URI.Type.TRACK:
				this.menuType = Cartify.ReactComponent.AlbumMenu;
				break;
		}
	...
	play(event) {
		Cartify.Player.playUri(this.uri, this.context);
	...
	closeButtonClicked(event) {
		...
		Cartify.Snackbar.enqueueCustomSnackbar
			? Cartify.Snackbar.enqueueCustomSnackbar("dismissed-release", {
					keyPrefix: "dismissed-release",
					children: Cartify.ReactComponent.Snackbar.wrapper({
						children: Cartify.ReactComponent.Snackbar.simpleLayout({
							leading: Cartify.ReactComponent.Snackbar.styledImage({
								src: this.props.imageURL,
								imageHeight: "24px",
								imageWidth: "24px",
							}),
							center: Cartify.React.createElement("div", {
								dangerouslySetInnerHTML: {
									__html: `Dismissed <b>${this.title}</b>.`,
								},
							}),
							trailing: Cartify.ReactComponent.Snackbar.ctaText({
								ctaText: "Undo",
			...
			: Cartify.showNotification(`Dismissed <b>${this.title}</b> from <br>${this.artist.name}</b>`);
	}
	...
	render() {
		...
		if (this.visual.count && this.trackCount) {
			detail.push(Cartify.Locale.get("tracklist-header.songs-counter", this.trackCount));
		}

		return react.createElement(
			Cartify.ReactComponent.RightClickMenu || "div",
			{
							...
							react.createElement(
								...
								react.createElement(
									"div",
									{
										...
										"aria-label": Cartify.Locale.get("play"),
										style: { "--size": "40px" },
