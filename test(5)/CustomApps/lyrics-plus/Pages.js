const CreditFooter = react.memo(({ provider, copyright }) => {
	...
	const credit = [Cartify.Locale.get("web-player.lyrics.providedBy", provider)];
	if (copyright) {
...
const SyncedLyricsPage = react.memo(({ lyrics = [], provider, copyright, isKara }) => {
	...
	useTrackPosition(() => {
		const newPos = Cartify.Player.getProgress();
						...
						: react.createElement(
								"p",
								{
									onContextMenu: (event) => {
										...
										Cartify.Platform.ClipboardAPI.copy(Utils.convertParsedToLRC(lyrics, belowMode).original)
											.then(() => Cartify.showNotification("Lyrics copied to clipboard"))
											.catch(() => Cartify.showNotification("Failed to copy lyrics to clipboard"));
									},
								},
					...
					belowMode &&
						react.createElement(
							"p",
							{
								...
								onContextMenu: (event) => {
									event.preventDefault();
									Cartify.Platform.ClipboardAPI.copy(Utils.convertParsedToLRC(lyrics, belowMode).conver)
										.then(() => Cartify.showNotification("Translated lyrics copied to clipboard"))
										.catch(() => Cartify.showNotification("Failed to copy translated lyrics to clipboard"));
								},
							},
							text
						)
...
class SearchBar extends react.Component {
	...
	componentDidMount() {
		...
		this.toggleCallback = () => {
			if (!(Cartify.Platform.History.location.pathname === "/lyrics-plus" && this.container)) return;
		...
		Cartify.Mousetrap().bind("mod+shift+f", this.toggleCallback);
		Cartify.Mousetrap(this.container).bind("mod+shift+f", this.toggleCallback);
		Cartify.Mousetrap(this.container).bind("enter", this.loopThroughCallback);
		Cartify.Mousetrap(this.container).bind("shift+enter", this.loopThroughCallback);
		Cartify.Mousetrap(this.container).bind("esc", this.unFocusCallback);
	}

	componentWillUnmount() {
		Cartify.Mousetrap().unbind("mod+shift+f", this.toggleCallback);
		Cartify.Mousetrap(this.container).unbind("mod+shift+f", this.toggleCallback);
		Cartify.Mousetrap(this.container).unbind("enter", this.loopThroughCallback);
		Cartify.Mousetrap(this.container).unbind("shift+enter", this.loopThroughCallback);
		Cartify.Mousetrap(this.container).unbind("esc", this.unFocusCallback);
	}
		...
		return react.createElement(
			...
			react.createElement("svg", {
				...
				dangerouslySetInnerHTML: {
					__html: Cartify.SVGIcons.search,
				},
			}),
			react.createElement(
				"span",
...
const SyncedExpandedLyricsPage = react.memo(({ lyrics, provider, copyright, isKara }) => {
	const [position, setPosition] = useState(() => Cartify.Player.getProgress() + CONFIG.visual["global-delay"] + CONFIG.visual.delay);
	const activeLineRef = useRef(null);
	const pageRef = useRef(null);

	useTrackPosition(() => {
		if (!Cartify.Player.data.is_paused) {
			setPosition(Cartify.Player.getProgress() + CONFIG.visual["global-delay"] + CONFIG.visual.delay);
		}
	});
			...
			return react.createElement(
				...
				react.createElement(
					"p",
					{
						onContextMenu: (event) => {
							event.preventDefault();
							Cartify.Platform.ClipboardAPI.copy(Utils.convertParsedToUnsynced(lyrics, belowMode).original)
								.then(() => Cartify.showNotification("Lyrics copied to clipboard"))
								.catch(() => Cartify.showNotification("Failed to copy lyrics to clipboard"));
						},
					},
				...
				belowMode &&
					react.createElement(
						"p",
						{
							...
							onContextMenu: (event) => {
								event.preventDefault();
								Cartify.Platform.ClipboardAPI.copy(Utils.convertParsedToUnsynced(lyrics, belowMode).conver)
									.then(() => Cartify.showNotification("Translated lyrics copied to clipboard"))
									.catch(() => Cartify.showNotification("Failed to copy translated lyrics to clipboard"));
							},
						},
		...
		const lyricsEl1 = react.createElement(
			...
			react.createElement("div", {
				...
				onContextMenu: (event) => {
					...
					Cartify.Platform.ClipboardAPI.copy(copylyrics)
						.then(() => Cartify.showNotification("Lyrics copied to clipboard"))
						.catch(() => Cartify.showNotification("Failed to copy lyrics to clipboard"));
				},
			})
