const Utils = {
	addQueueListener(callback) {
		Cartify.Player.origin._events.addListener("queue_update", callback);
	},
	removeQueueListener(callback) {
		Cartify.Player.origin._events.removeListener("queue_update", callback);
	},
			...
			if (line.trim() !== "") {
				if (isKaraoke) {
					if (!lyricContent.endsWith(">")) {
						...
						const endTime = lines[i + 1]?.match(syncedTimestamp)?.[1] || this.formatTime(Number(Cartify.Player.data.item.metadata.duration));
