...
const RefreshTokenButton = ({ setTokenCallback }) => {
	...
	useEffect(() => {
		if (buttonText === "Refreshing token...") {
			Cartify.CosmosAsync.get("https://apic-appmobile.musixmatch.com/ws/1.1/token.get?app_id=mac-ios-v2.0", null, {
...
const ConfigHotkey = ({ name, defaultValue, onChange = () => {} }) => {
	const [value, setValue] = useState(defaultValue);
	const [trap] = useState(new Cartify.Mousetrap());
	...
	return react.createElement(
		...
		react.createElement(
			...
			react.createElement(
				...
				react.createElement(SwapButton, {
					icon: Cartify.SVGIcons["chart-up"],
					onClick: () => onSwap(item.name, -1),
					disabled: isFirst,
				}),
				react.createElement(SwapButton, {
					icon: Cartify.SVGIcons["chart-down"],
					onClick: () => onSwap(item.name, 1),
					disabled: isLast,
				}),
				react.createElement(ButtonSVG, {
...
const corsProxyTemplate = () => {
	const [proxyValue, setProxyValue] = react.useState(localStorage.getItem("Cartify:corsProxyTemplate") || "https://cors-proxy.cartify.app/{url}");
	...
	return react.createElement("input", {
		...
		onChange: (event) => {
			...
			if (value === "" || !value) return localStorage.removeItem("Cartify:corsProxyTemplate");
			localStorage.setItem("Cartify:corsProxyTemplate", value);
		},
	});
};
...
function openConfig() {
	const configContainer = react.createElement(
		...
		react.createElement("span", {
			dangerouslySetInnerHTML: {
				__html: "Spotify will reload its webview after applying. Leave empty to restore default: <code>https://cors-proxy.cartify.app/{url}</code>",
			},
		})
	);
