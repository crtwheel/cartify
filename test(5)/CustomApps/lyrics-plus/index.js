...
/** @type {React} */
const react = Cartify.React;
const { useState, useEffect, useCallback, useMemo, useRef } = react;
/** @type {import("react").ReactDOM} */
const spotifyVersion = Cartify.Platform.version;
...
class LyricsContainer extends react.Component {
	...
	async fetchColors(uri) {
		...
		try {
			try {
				const { fetchExtractedColorForTrackEntity } = Cartify.GraphQL.Definitions;
				const { data } = await Cartify.GraphQL.Request(fetchExtractedColorForTrackEntity, { uri });
				const { hex } = data.trackUnion.albumOfTrack.coverArt.extractedColors.colorDark;
				vibrant = Number.parseInt(hex.replace("#", ""), 16);
			} catch {
				const colors = await Cartify.CosmosAsync.get(`https://spclient.wg.spotify.com/colorextractor/v1/extract-presets?uri=${uri}&format=json`);
	...
	async fetchTempo(uri) {
		const audio = await Cartify.CosmosAsync.get(
	...
	async refreshMusixmatchTranslation() {
		...
		Cartify.showNotification(MUSIXMATCH_TRANSLATION_FETCH_MESSAGE, false, 1000);
		...
		try {
			...
			console.error(error);
			if (isLatestRequest()) {
				Cartify.showNotification(MUSIXMATCH_TRANSLATION_FETCH_FAILED_MESSAGE, true, 3000);
				if (CACHE[currentUri]) {
					CACHE[currentUri].musixmatchTranslation = null;
					CACHE[currentUri].musixmatchTranslationLanguage = null;
				}
	...
	async translateLyrics(language, lyrics, targetConvert) {
		...
		Cartify.showNotification("Converting...", false, 1000);
		if (!this.translator) {
		...
		try {
			if (language === "ja") {
				...
				if (targetConvert === "cn") {
					Cartify.showNotification("No conversion is needed", false, 1000);
					return lyrics;
				}
			...
			Cartify.showNotification("Converting...", false, 0);
			return res;
		} catch (error) {
			Cartify.showNotification("Convert Error!", true);
			console.error(error);
		}
	}

	resetDelay() {
		CONFIG.visual.delay = Number(localStorage.getItem(`lyrics-delay:${Cartify.Player.data.item.uri}`)) || 0;
	}
	...
	processLyricsFromFile(event) {
		...
		if (file[0].size > 1024 * 1024) {
			Cartify.showNotification("File too large", true);
			return;
		}
		...
		reader.onload = (e) => {
			try {
				...
				if (!parsedKeys.length) {
					Cartify.showNotification("Nothing to load", true);
					return;
				}
				...
				Cartify.showNotification(`Loaded ${parsedKeys.join(", ")} lyrics from file`);
			} catch (e) {
				console.error(e);
				Cartify.showNotification("Failed to load lyrics", true);
			}
		};

		reader.onerror = (e) => {
			console.error(e);
			Cartify.showNotification("Failed to read file", true);
		};
	...
	initMoustrap() {
		if (!this.mousetrap && Cartify.Mousetrap) {
	...
	componentDidMount() {
		...
		if (Cartify.Player?.data?.item) {
			this.state.explicitMode = this.state.lockMode;
			this.currentTrackUri = Cartify.Player.data.item.uri;
			this.fetchLyrics(Cartify.Player.data.item, this.state.explicitMode);
		}
		...
		reloadLyrics = () => {
			...
			this.updateVisualOnConfigChange();
			this.forceUpdate();
			this.fetchLyrics(Cartify.Player.data.item, this.state.explicitMode, true);
		};
		...
		this.configButton = new Cartify.Menu.Item("Lyrics Plus config", false, openConfig, "lyrics");
	...
	render() {
					...
					react.createElement(
						...
						react.createElement("svg", {
							...
							dangerouslySetInnerHTML: {
								__html: Cartify.SVGIcons[this.state.isCached ? "downloaded" : "download"],
							},
						})
					)
				),
				react.createElement(
					...
					react.createElement(
						...
						react.createElement("svg", {
							...
							fill: "currentColor",
							dangerouslySetInnerHTML: {
								__html: Cartify.SVGIcons["plus-alt"],
							},
						})
					)
				)
				...
				react.createElement(TopBarContent, {
					...
					switchCallback: (label) => {
						...
						if (mode !== this.state.mode) {
							...
							const info = this.infoFromTrack(Cartify.Player.data.item);
							if (info?.uri && CACHE[info?.uri]) {
								CACHE[info.uri].mode = mode;
							}

							this.setState({ explicitMode: mode });
							this.state.provider !== "local" && this.fetchLyrics(Cartify.Player.data.item, mode);
						}
					},
					lockCallback: (label) => {
						...
						this.fetchLyrics(Cartify.Player.data.item, mode);
						CONFIG.locked = mode;
		...
		if (this.state.isFullscreen) return Cartify.ReactDOM.createPortal(out, this.fullscreenContainer);
		if (fadLyricsContainer) return Cartify.ReactDOM.createPortal(out, fadLyricsContainer);
		return out;
	}
}
