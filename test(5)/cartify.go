...
func init() {
	...
	if isAdmin.Check(bypassAdminCheck) {
		utils.PrintError("cartify should NOT be run with administrator or root privileges")
...
func main() {
		...
		path, err := (func() (string, error) {
			if styleFocus {
				...
				if len(commands) == 0 && len(flags) == 0 {
					...
					return utils.GetCartifyFolder(), nil
				}
	...
	utils.PrintBold("cartify v" + version)
	if slices.Contains(commands, "upgrade") || slices.Contains(commands, "update") {
		...
		spotStat := spotifystatus.Get(spotifyPath)
		if spotStat.IsBackupable() {
			utils.PrintNote("cartify is up-to-date! If you ran this because Cartify disappeared after Spotify updated, we'll attempt to fix it for you right now.")
...
Run "cartify -h" for a list of valid commands.`))
		}
	}
...
func help() {
	...
	log.Println(utils.Bold("USAGE") + "\n" +
		"cartify [-q] [-e] [-a] \x1B[4mcommand\033[0m...\n" +
		"cartify {-c | --config} | {-v | --version} | {-h | --help}\n\n" +
...
install             Automatically set up Cartify: applies customization,
                    creates scheduled task for auto-recovery, and more.
                    Run this once after installing Cartify.
...
path                Print path of Spotify's executable, userdata, and more.
                    ...
                    Cartify path userdata

                    3. Print all paths:
                    Cartify path all

                    4. Toggle focus with flags:
                    Cartify path <flag> <option>
...
config              1. Print all config fields and values:
                    ...
                    Cartify config <field>

                    Example usage:
                    Cartify config color_scheme
                    Cartify config custom_apps

                    3. Change value of one or multiple config fields.
                    Cartify config <field> <value> [<field> <value> ...]
                    ...
                    Cartify config disable_sentry 1
                    - Add extension "myFakeExt.js" to current extensions list:
                    Cartify config extensions myFakeExt.js
                    - Remove extension "wrongname.js" from extensions list:
                    Cartify config extensions wrongname.js-
                    - Disable "inject_css" and enable "song_page"
                    Cartify config inject_css 0 song_page 1
...
color               1. Print all color fields and values.
                    ...
                    Cartify color <field> <value> [<field> <value> ...]

                    <value> can be in hex or decimal (rrr,ggg,bbb) format.

                    Example usage:
                    - Change main to ff0000
                    Cartify color main ff0000
                    - Change sidebar to 00ff00 and button to 0000ff
                    Cartify color sidebar 00ff00 button 0000ff

config-dir          Show config directory in file viewer

upgrade|update      Update Cartify to the latest version if an update is available
...
For config information, run "cartify -h config".
For more information and reporting bugs: https://github.com/crtwheel/cartify/`)
}
...
spotify_launch_flags <string>
    ...
    List of valid flags: https://cartify.app/docs/development/spotify-cli-flags

always_enable_devtools <0 | 1>
    Whether Chrome DevTools is enabled when launching/restarting Spotify.

check_cartify_update <0 | 1>
    Whether to always check for updates when running Cartify.
...
expose_apis <0 | 1>
    Leaks some Spotify's API, functions, objects to Cartify global object that
