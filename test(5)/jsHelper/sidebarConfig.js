(function SidebarConfig() {
	...
	function writeStorage() {
		...
		return localStorage.setItem("Cartify-sidebar-config:ylx", JSON.stringify(array));
	}

	const container = document.createElement("div");
	container.id = "Cartify-sidebar-config";
...
#Cartify-sidebar-config button:disabled {
...
`;
	...
	function injectInteraction() {
		...
		YLXSidebarState = Cartify.Platform.LocalStorageAPI.getItem("ylx-sidebar-state");
	...
	function removeInteraction() {
		...
		if (YLXSidebarState === 1) document.querySelector(".main-yourLibraryX-collapseButton > button")?.click();
			...
			document.documentElement.style.setProperty(
				"--nav-bar-width",
				`${Cartify.Platform.LocalStorageAPI.getItem(
	...
	(async () => {
		...
		while (!Cartify.Snackbar?.enqueueCustomSnackbar) {
			await new Promise((resolve) => setTimeout(resolve, 10));
		}

		if (document.querySelector(".Root__globalNav")) {
			const content = Cartify.React.createElement("div", {
				dangerouslySetInnerHTML: {
					__html:
						"Sidebar config is not supported when Global Navbar is enabled.<br>In your terminal, please run <code>Cartify config sidebar_config 0</code> command and then re-apply Cartify with <code>cartify.apply</code>.",
				},
				style: {
					"text-size": "12px",
				},
			...
			Cartify.Snackbar?.enqueueCustomSnackbar("sidebar-config", {
				keyPrefix: "sidebar-config",
				autoHideDuration: 7500,
				children: Cartify.ReactComponent.Snackbar.wrapper({
					children: Cartify.React.createElement(Cartify.ReactComponent.Snackbar.simpleLayout, {
						center: content,
						children: content,
						dragMetadata: {},
					}),
	...
	function InitSidebarXConfig() {
		...
		appItems.id = "Cartify-sticky-list";
		// SHOW container
		list = document.createElement("ul");
		list.id = "Cartify-show-list";
		// HIDDEN container
		hiddenList = document.createElement("ul");
		hiddenList.id = "Cartify-hidden-list";
		hiddenList.classList.add("hidden-visually");
		const playlistList = libraryItems.querySelector("ul");
		playlistList.id = "Cartify-playlist-list";
		...
		let storage = [];
		try {
			storage = JSON.parse(localStorage.getItem("Cartify-sidebar-config:ylx"));
		...
		const observer = new MutationObserver((mutations) => {
			for (const mutation of mutations) {
				if (mutation.type === "childList" && mutation.addedNodes.length) {
					mutation.addedNodes[0].id = "Cartify-playlist-list";
				}
			}
		});
...
.Root__nav-bar :not(.main-yourLibraryX-entryPoints) > #Cartify-show-list >* {
padding: 0 24px 0 8px;
}
.main-yourLibraryX-entryPoints #Cartify-show-list,
.main-yourLibraryX-entryPoints #Cartify-hidden-list {
padding: 0 12px;
}
`;
