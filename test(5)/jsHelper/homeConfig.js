...
(async () => {
	...
	CartifyHomeConfig.arrange = (sections) => {
		mounted = true;
		if (list) {
			return list;
		}
		const stickList = (localStorage.getItem("Cartify-home-config:stick") || "").split(",");
		const lowList = (localStorage.getItem("Cartify-home-config:low") || "").split(",");
		const stickSections = [];
		const lowSections = [];
...
#Cartify-home-config button:disabled {
    color: var(--spice-button-disabled);
}
`;

	const container = document.createElement("div");
	container.id = "Cartify-home-config";
	...
	function injectInteraction() {
		...
		function appendItems() {
			...
			localStorage.setItem(
				"Cartify-home-config:stick",
				stick.map((a) => a.dataset.uri)
			);
			localStorage.setItem(
				"Cartify-home-config:low",
	...
	CartifyHomeConfig.menu = new Cartify.Menu.Item(
		"Home config",
		false,
		(self) => {
		...
		Cartify.SVGIcons["grid-view"]
	);

	CartifyHomeConfig.addToMenu = () => {
		CartifyHomeConfig.menu.register();
	};
	CartifyHomeConfig.removeMenu = () => {
		CartifyHomeConfig.menu.setState(false);
		CartifyHomeConfig.menu.deregister();
	};

	await new Promise((res) => Cartify.Events.platformLoaded.on(res));
	// Init
	if (Cartify.Platform.History.location.pathname === "/") {
		CartifyHomeConfig.addToMenu();
	}

	Cartify.Platform.History.listen(({ pathname }) => {
		if (pathname === "/") {
			CartifyHomeConfig.addToMenu();
		} else {
			CartifyHomeConfig.removeMenu();
