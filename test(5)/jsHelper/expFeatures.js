(async () => {
	...
	try {
		overrideList = JSON.parse(localStorage.getItem("Cartify-exp-features"));
	...
	Cartify.expFeatureOverride = (feature) => {
		hooksPatched = true;
		...
		localStorage.setItem("Cartify-exp-features", JSON.stringify(overrideList));
		return feature;
	};

	const content = document.createElement("div");
	content.classList.add("Cartify-exp-features");
	const style = document.createElement("style");
	style.innerHTML = `
.Cartify-exp-features .col {
    padding: 0;
}
.Cartify-exp-features .setting-row::after {
    content: "";
    display: table;
    clear: both;
}
.Cartify-exp-features .setting-row {
    display: flex;
    padding: 10px 0;
    align-items: center;
}
.Cartify-exp-features .setting-row .col.description {
    float: left;
    padding-right: 15px;
    width: 100%;
}
.Cartify-exp-features .setting-row .col.action {
    float: right;
    text-align: right;
}
.Cartify-exp-features .setting-row .col.action .dropdown {
	width: max-content;
}
.Cartify-exp-features button.switch {
    align-items: center;
    border: 0px;
    border-radius: 50%;
...
.Cartify-exp-features button.switch.disabled,
.Cartify-exp-features button.switch[disabled] {
    color: rgba(var(--spice-rgb-text), .3);
}
.Cartify-exp-features button.reset {
	font-weight: 700;
	font-size: medium;
...
.Cartify-exp-features button.reset:hover {
	transform: scale(1.04);
	border-color: var(--spice-text);
}
.Cartify-exp-features .search-container {
    width: 100%;
}
.Cartify-exp-features .setting-row#search .col.action {
    position: relative;
    width: 100%;
}
.Cartify-exp-features .setting-row#search svg {
    position: absolute;
    margin: 12px;
}
.Cartify-exp-features input.search {
    border-style: solid;
	...
	(function waitForRemoteConfigResolver() {
		...
		if (!hooksPatched || (!Cartify.RemoteConfigResolver && !Cartify.Platform?.RemoteConfigDebugAPI && !Cartify.Platform?.RemoteConfiguration)) {
			setTimeout(waitForRemoteConfigResolver, 500);
			return;
		}

		let remoteConfiguration = Cartify.RemoteConfigResolver?.value.remoteConfiguration || Cartify.Platform?.RemoteConfiguration;
		const setOverrides = async (overrides) => {
			if (Cartify.Platform?.RemoteConfigDebugAPI) {
				for (const [name, value] of Object.entries(overrides)) {
					const feature = overrideList[name];
					const type = feature.values ? "enum" : typeof value === "number" ? "number" : "boolean";
					await Cartify.Platform.RemoteConfigDebugAPI.setOverride(
						{
							source: "web",
							type,
							name,
			...
			} else if (Cartify.RemoteConfigResolver?.value?.setOverrides) {
				Cartify.RemoteConfigResolver.value.setOverrides(Cartify.createInternalMap?.(overrides));
			}
		};

		(async function waitForResolver() {
			if (!Cartify.RemoteConfigResolver && !Cartify.Platform?.RemoteConfigDebugAPI) {
				isFallback = true;
			...
			notice.remove();
			remoteConfiguration =
				Cartify?.RemoteConfigResolver?.value.remoteConfiguration ?? (await Cartify.Platform?.RemoteConfigDebugAPI.getProperties());
		})();

		for (const key of Object.keys(overrideList)) {
			if (newFeatures.includes(key)) continue;
			delete overrideList[key];
			console.warn(`[Cartify-exp-features] Removed ${key} from override list`);
			localStorage.setItem("Cartify-exp-features", JSON.stringify(overrideList));
		}
...
<input type="text" class="search" placeholder="Search for a feature">
		...
		resetBtn.onclick = () => {
			localStorage.removeItem("Cartify-exp-features");
