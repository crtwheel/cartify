window.Cartify = {
	Player: {
		addEventListener: (type, callback) => {
			if (!(type in Cartify.Player.eventListeners)) {
				Cartify.Player.eventListeners[type] = [];
			}
			Cartify.Player.eventListeners[type].push(callback);
		},
		dispatchEvent: (event) => {
			if (!(event.type in Cartify.Player.eventListeners)) {
				return true;
			}
			const stack = Cartify.Player.eventListeners[event.type];
		...
		eventListeners: {},
		seek: (p) => {
			const duration = p <= 1 ? Math.round(p * Cartify.Player.origin._state.duration) : p;
			Cartify.Player.origin.seekTo(duration);
		},
		getProgress: () =>
			(Cartify.Player.origin._state.isPaused ? 0 : Date.now() - Cartify.Player.origin._state.timestamp) +
			Cartify.Player.origin._state.positionAsOfTimestamp,
		getProgressPercent: () => Cartify.Player.getProgress() / Cartify.Player.origin._state.duration,
		getDuration: () => Cartify.Player.origin._state.duration,
		setVolume: (v) => {
			Cartify.Platform.PlaybackAPI.setVolume(v);
		},
		increaseVolume: () => {
			Cartify.Platform.PlaybackAPI.raiseVolume();
		},
		decreaseVolume: () => {
			Cartify.Platform.PlaybackAPI.lowerVolume();
		},
		getVolume: () => Cartify.Platform.PlaybackAPI._volume,
		next: () => {
			Cartify.Player.origin.skipToNext();
		},
		back: () => {
			Cartify.Player.origin.skipToPrevious();
		},
		togglePlay: () => {
			Cartify.Player.isPlaying() ? Cartify.Player.pause() : Cartify.Player.play();
		},
		isPlaying: () => !Cartify.Player.origin._state.isPaused,
		toggleShuffle: () => {
			Cartify.Player.origin.setShuffle(!Cartify.Player.origin._state.shuffle);
		},
		getShuffle: () => Cartify.Player.origin._state.shuffle,
		setShuffle: (b) => {
			Cartify.Player.origin.setShuffle(b);
		},
		toggleRepeat: () => {
			Cartify.Player.origin.setRepeat((Cartify.Player.origin._state.repeat + 1) % 3);
		},
		getRepeat: () => Cartify.Player.origin._state.repeat,
		setRepeat: (r) => {
			Cartify.Player.origin.setRepeat(r);
		},
		getMute: () => Cartify.Player.getVolume() === 0,
		toggleMute: () => {
			Cartify.Player.setMute(!Cartify.Player.getMute());
		},
		setMute: (b) => {
			if (b !== Cartify.Player.getMute()) {
		...
		getHeart: () => Cartify.Player.origin._state.item.metadata["collection.in_collection"] === "true",
		pause: () => {
			Cartify.Player.origin.pause();
		},
		play: () => {
			Cartify.Player.origin.resume();
		},
		playUri: async (uri, context = {}, options = {}) => {
			return await Cartify.Player.origin.play({ uri: uri }, context, options);
		},
		removeEventListener: (type, callback) => {
			if (!(type in Cartify.Player.eventListeners)) return;
			const stack = Cartify.Player.eventListeners[type];
			for (let i = 0; i < stack.length; i++) {
				if (stack[i] === callback) {
					stack.splice(i, 1);
					return;
		...
		skipBack: (amount = 15e3) => {
			Cartify.Player.origin.seekBackward(amount);
		},
		skipForward: (amount = 15e3) => {
			Cartify.Player.origin.seekForward(amount);
		},
		setHeart: (b) => {
			const uris = [Cartify.Player.origin._state.item.uri];
			if (b) {
				Cartify.Platform.LibraryAPI.add({ uris });
			} else {
				Cartify.Platform.LibraryAPI.remove({ uris });
			}
		},
		toggleHeart: () => {
			Cartify.Player.setHeart(!Cartify.Player.getHeart());
		},
	},
	test: () => {
		function checkObject(object) {
			...
			for (const method of methods) {
				if (objectToCheck[method] === undefined || objectToCheck[method] === null) {
					console.error(`${name}.${method} is not available. Please open an issue in the Cartify repository to inform us about it.`);
					count--;
				}
			}
		...
		const objectsToCheck = new Set([
			...
			{
				objectToCheck: Cartify.ReactComponent,
				name: "Cartify.ReactComponent",
				methods: new Set([
					"RightClickMenu",
					"ContextMenu",
					"Menu",
			...
			{
				objectToCheck: Cartify.ReactComponent.Cards,
				name: "Cartify.ReactComponent.Cards",
				methods: new Set([
					"Default",
					"Hero",
					"CardImage",
			...
			{
				objectToCheck: Cartify.ReactHook,
				name: "Cartify.ReactHook",
...
(function waitForPlatform() {
	if (!Cartify._platform) {
		setTimeout(waitForPlatform, 50);
		return;
	}
	const { _platform } = Cartify;
	for (const key of Object.keys(_platform)) {
		if (key.startsWith("get") && typeof _platform[key] === "function") {
			Cartify.Platform[key.slice(3)] = _platform[key]();
		} else {
			Cartify.Platform[key] = _platform[key];
		}
	}
})();

(function addMissingPlatformAPIs() {
	if (!Cartify.Platform?.version && !Cartify.Platform?.Registry) {
		setTimeout(addMissingPlatformAPIs, 50);
		return;
	}
	const os = Cartify.Platform.operatingSystem;
	const version = Cartify.Platform.version.split(".").map((i) => Number.parseInt(i, 10));
	if (version[0] === 1 && version[1] === 2 && version[2] < 38) return;

	for (const [key, _] of Cartify.Platform.Registry._map.entries()) {
		...
		if (Object.hasOwn(Cartify.Platform, symbolName)) continue;
		try {
			const resolvedAPI = Cartify.Platform.Registry.resolve(key);
			Cartify.Platform[symbolName] = resolvedAPI;

			console.debug(`[CartifyWrapper] Resolved PlatformAPI from Registry: ${symbolName}`);
		} catch (err) {
			console.error(`[CartifyWrapper] Error resolving PlatformAPI from Registry: ${symbolName}`, err);
		}
	}
})();
...
(async function addProxyCosmos() {
	if (!Cartify.Player.origin?._cosmos && !Cartify.Platform?.Registry) {
		setTimeout(addProxyCosmos, 50);
		return;
	}

	const _cosmos = Cartify.Player.origin?._cosmos ?? Cartify.Platform?.Registry.resolve(Symbol.for("Cosmos"));
	...
	const handler = {
		get: (target, prop, receiver) => {
			...
			return async function (url, body) {
				...
				const corsProxyURLTemplate = window.localStorage.getItem("Cartify:corsProxyTemplate") ?? "https://cors-proxy.cartify.app/{url}";
				...
				if (shouldUseCORSProxy) {
					...
					try {
						...
						console.error("[CartifyWrapper] Invalid CORS Proxy URL template");
					}
				}

				const Authorization = `Bearer ${Cartify.Platform.AuthorizationAPI.getState().token.accessToken}`;
				let injectedHeaders = {};
				if (isWebAPI) injectedHeaders = { Authorization };
				if (isSpClientAPI) {
					injectedHeaders = {
						Authorization,
						"Spotify-App-Version": Cartify.Platform.version,
						"App-Platform": Cartify.Platform.PlatformData.app_platform,
					};
				}
	...
	while (!Cartify.Player.origin) await new Promise((r) => setTimeout(r, 50));
	Cartify.Player.origin._cosmos = new Proxy(_cosmos, handler);
	Object.defineProperty(Cartify, "CosmosAsync", {
		get: () => {
			return Cartify.Player.origin?._cosmos;
		},
	});
})();
...
(async function hotloadWebpackModules() {
	...
	console.log("[CartifyWrapper] Waiting for required webpack modules to load");
	...
	Cartify.React = cache.find((m) => m?.useMemo);

	while (!webpackDidCallback) {
		await new Promise((r) => setTimeout(r, 100));
	}
	console.log("[CartifyWrapper] All required webpack modules loaded");
	chunks = Object.entries(require.m);
	cache = Object.keys(require.m).map((id) => require(id));
	Cartify.Events.platformLoaded.fire();
	...
	const _ScrollableContainer = (() => {
		...
		function injectStyles() {
			...
			style.className = "Cartify-scrollable-container";
			style.textContent = `
.Cartify-sc-contentArea { overflow: hidden; position: relative; }
.Cartify-sc-scroller { display: flex; align-items: center; overflow-x: auto; scrollbar-width: none; white-space: nowrap; width: 100%; -ms-overflow-style: none; overscroll-behavior-x: contain; will-change: transform; }
@media (prefers-reduced-motion: no-preference) { .Cartify-sc-scroller { scroll-behavior: smooth; } }
.Cartify-sc-scroller::-webkit-scrollbar { display: none; }
.Cartify-sc-scroller.Cartify-sc-snap { scroll-snap-type: inline mandatory; }
.Cartify-sc-scroller.Cartify-sc-snap .Cartify-sc-snapCenter [data-carousel-item] { scroll-snap-align: center; }
.Cartify-sc-scroller.Cartify-sc-snap .Cartify-sc-snapStart [data-carousel-item] { scroll-snap-align: start; }
.Cartify-sc-scroller.Cartify-sc-wheelEnabled { overscroll-behavior: contain; }
.Cartify-sc-scroller.Cartify-sc-maskGradient { --sc-start-color: #000; --sc-end-color: #000; -webkit-mask-composite: source-in, xor; mask-composite: intersect; -webkit-mask-image: linear-gradient(90deg, var(--sc-start-color) 0, #000 120px), linear-gradient(90deg, #000 calc(100% - 120px), var(--sc-end-color) 100%); mask-image: linear-gradient(90deg, var(--sc-start-color) 0, #000 120px), linear-gradient(90deg, #000 calc(100% - 120px), var(--sc-end-color) 100%); -webkit-mask-size: 100% 100%; mask-size: 100% 100%; }
.Cartify-sc-scroller.Cartify-sc-maskGradient.Cartify-sc-maskStart { --sc-start-color: transparent; }
.Cartify-sc-scroller.Cartify-sc-maskGradient.Cartify-sc-maskEnd { --sc-end-color: transparent; }
.Cartify-sc-linearGradient::before, .Cartify-sc-linearGradient::after { bottom: 0; content: ""; height: 100%; opacity: 0; pointer-events: none; position: absolute; top: 0; transition: opacity .15s ease-out; width: 120px; z-index: 2; }
.Cartify-sc-linearGradient::before { background: linear-gradient(90deg, var(--carousel-start-chevron-gradient, var(--spice-main)) 0, transparent 100%); inset-inline-start: 0; }
.Cartify-sc-linearGradient::after { background: linear-gradient(-90deg, var(--carousel-end-chevron-gradient, var(--spice-main)) 0, transparent 100%); inset-inline-end: 0; }
.Cartify-sc-linearGradient.Cartify-sc-lgStart::before { opacity: 1; }
.Cartify-sc-linearGradient.Cartify-sc-lgEnd::after { opacity: 1; }
.Cartify-sc-carousel { bottom: 0; left: 0; position: absolute; right: 0; top: 0; justify-content: space-between; align-items: center; display: flex; pointer-events: none; }
.Cartify-sc-chevronBtn { display: flex; border: none; border-radius: 50%; cursor: pointer; justify-content: center; align-items: center; backdrop-filter: var(--chevrons-button-backdrop-filter, none); background: transparent; background-color: var(--chevrons-button-color, var(--background-elevated-base)); height: 24px; opacity: 0; position: relative; transition: color .15s ease-out, opacity .15s ease-out, background-color .15s ease-out, translate .15s ease-out; translate: 0; width: 24px; z-index: 3; pointer-events: none; color: var(--text-base, #fff); }
.Cartify-sc-chevronBtn > * { opacity: .7; z-index: 2; }
.Cartify-sc-chevronBtn:hover { background-color: var(--chevrons-button-hover-color, var(--background-elevated-highlight)); }
.Cartify-sc-chevronBtn:hover > * { opacity: 1; }
.Cartify-sc-chevronBtn.Cartify-sc-chevronVisible { opacity: 1; pointer-events: auto; }
.Cartify-sc-onHover .Cartify-sc-chevronBtn { opacity: 0; }
.Cartify-sc-contentArea:hover .Cartify-sc-onHover .Cartify-sc-chevronBtn.Cartify-sc-chevronVisible { opacity: 1; }
.Cartify-sc-contentArea:hover .Cartify-sc-onHover .Cartify-sc-chevronStart.Cartify-sc-chevronVisible { translate: 8px; }
.Cartify-sc-contentArea:hover .Cartify-sc-onHover .Cartify-sc-chevronEnd.Cartify-sc-chevronVisible { translate: -8px; }
.Cartify-sc-scroller > div[role="presentation"] > button { margin-inline-start: 0px !important; }
body[data-dragging-uri-type] .Cartify-sc-chevronBtn { pointer-events: none; }`;
			document.head.appendChild(style);
		}

		function useDragToScroll({ isDisabled = true } = {}) {
			const { useRef, useCallback } = Cartify.React;
		...
		function useScrollState(scrollerRef, contentRef) {
			const { useState, useCallback, useEffect } = Cartify.React;
		...
		function ScrollableContainerComponent(props) {
			const { useRef, useCallback, useMemo } = Cartify.React;
			const h = Cartify.ReactJSX.jsx;
			const hsf = Cartify.ReactJSX.jsxs;
			const cn = Cartify.classnames;
			...
			const makeChevron = (svgPath, position, visible, dir) =>
				h("div", {
					className: cn("Cartify-sc-chevronBtn", `Cartify-sc-chevron${position}`, { "Cartify-sc-chevronVisible": visible }),
					onClick: (e) => {
						e.preventDefault();
						e.stopPropagation();
						navigate(dir);
			...
			return hsf("div", {
				className: cn("Cartify-sc-contentArea", className, {
					"Cartify-sc-linearGradient": isLinearGradient,
					"Cartify-sc-lgStart": isLinearGradient && canGoStart,
					"Cartify-sc-lgEnd": isLinearGradient && canGoEnd,
				}),
				children: [
					h("div", {
						ref: scrollerRef,
						className: cn("Cartify-sc-scroller", scrollerClassName, {
							"Cartify-sc-snap": isSnap,
							"Cartify-sc-maskGradient": isMask,
							"Cartify-sc-wheelEnabled": wheelScrollEnabled,
							"Cartify-sc-maskStart": isMask && canGoStart,
							"Cartify-sc-maskEnd": isMask && canGoEnd,
						}),
						...
						children: h("div", {
							...
							className: cn(scrollContentClassName, {
								"Cartify-sc-snapStart": scrollSnapAlign === "start",
								"Cartify-sc-snapCenter": scrollSnapAlign === "center",
							}),
							children,
						}),
					}),
					showButtons !== SHOW_BUTTONS.NEVER &&
						hsf("div", {
							className: cn("Cartify-sc-carousel", chevronsClassName, {
								"Cartify-sc-onHover": showButtons === SHOW_BUTTONS.ON_HOVER,
							}),
	...
	Object.assign(Cartify, {
		...
		Player: {
			...Cartify.Player,
			get origin() {
				return Cartify.Platform?.PlayerAPI;
			},
		},
		GraphQL: {
			...Cartify.GraphQL,
			get Request() {
				return Cartify.Platform?.GraphQLLoader || Cartify.GraphQL.Handler?.(Cartify.GraphQL.Context);
			},
	...
	if (!Cartify.ContextMenuV2._context) Cartify.ContextMenuV2._context = Cartify.React.createContext({});
	...
	(function waitForChunks() {
		...
		if (listOfComponents.every((component) => component.split(".").reduce((o, k) => o?.[k], Cartify.ReactComponent) !== undefined)) return;
		...
		Object.assign(Cartify.ReactComponent.Cards, Object.fromEntries(lazyCards));

		Cartify.ReactComponent.Slider = wrapProvider(functionModules.find((m) => fnStr(m).includes("progressBarRef")));
		Cartify.ReactComponent.Toggle = functionModules.find((m) => fnStr(m).includes("onSelected") && fnStr(m).includes('type:"checkbox"'));

		// chunks
		const dropdownChunk = currentChunks.find(([, value]) => fnStr(value).includes("dropdown-list") && fnStr(value).includes('"listbox"'));
		if (dropdownChunk) {
			Cartify.ReactComponent.Dropdown =
		...
		if (toggleChunk && !Cartify.ReactComponent.Toggle) {
			Cartify.ReactComponent.Toggle = Object.values(require(toggleChunk[0]))[0].render;
		}

		if (!listOfComponents.every((component) => component.split(".").reduce((o, k) => o?.[k], Cartify.ReactComponent) !== undefined)) {
			setTimeout(waitForChunks, 100);
			return;
		}

		if (Cartify.ReactComponent.ScrollableContainer) setTimeout(refreshNavLinks, 100);
	})();

	(function waitForSnackbar() {
		if (!Object.keys(Cartify.Snackbar).length) {
	...
	if (Cartify.Locale) Cartify.Locale._supportedLocales = cache.find((m) => typeof m?.ja === "string");

	Object.defineProperty(Cartify, "Queue", {
		get() {
			return Cartify.Player.origin?._queue?._state ?? Cartify.Player.origin?._queue?._queue;
		},
	});
	...
	if (!Cartify.ReactComponent?.ConfirmDialog && confirmDialogChunk) {
		Cartify.ReactComponent.ConfirmDialog = Object.values(require(confirmDialogChunk[0])).find((m) => typeof m === "object");
	} else {
		Cartify.ReactComponent.ConfirmDialog = functionModules.find(
	...
	if (contextMenuChunk) {
		Cartify.ReactComponent.ContextMenu = Object.values(require(contextMenuChunk[0])).find((m) => typeof m === "function");
	}
	...
	if (playlistMenuChunk && !Cartify.ReactComponent?.PlaylistMenu) {
		Cartify.ReactComponent.PlaylistMenu = Object.values(require(playlistMenuChunk[0])).find(
	...
	if (infiniteQueryChunk) {
		Cartify.ReactQuery.useInfiniteQuery = Object.values(require(infiniteQueryChunk[0])).find((m) => typeof m === "function");
	}

	if (Cartify.Color) Cartify.Color.CSSFormat = modules.find((m) => m?.RGBA);

	// Combine snackbar and notification
	(function bindShowNotification() {
		if (!Cartify.Snackbar?.enqueueSnackbar && !Cartify.showNotification) {
			setTimeout(bindShowNotification, 250);
			return;
		}

		if (Cartify.Snackbar?.enqueueSnackbar) {
			Cartify.showNotification = (message, isError, msTimeout) => {
				Cartify.Snackbar.enqueueSnackbar(message, {
		...
		Cartify.Snackbar.enqueueSnackbar = (message, { variant = "default", autoHideDuration } = {}) => {
			isError = variant === "error";
			Cartify.showNotification(message, isError, autoHideDuration);
		};
	})();

	// Image color extractor
	(async function bindColorExtractor() {
		if (!Cartify.GraphQL.Request) {
		...
		Cartify.extractColorPreset = async (image) => {
			const analysis = await imageAnalysis(Cartify.GraphQL.Request, image);
	...
	function wrapProvider(component) {
		...
		return (props) =>
			Cartify.React.createElement(
				Cartify.ReactComponent.RemoteConfigProvider,
				{ configuration: Cartify.Platform.RemoteConfiguration },
				Cartify.React.createElement(component, props)
			);
	}
	...
	(function waitForURI() {
		...
		// URI.Type
		Cartify.URI.Type = URIModules.find((m) => m?.PLAYLIST_V2);

		// Parse functions
		Cartify.URI.from = URIModules.find((m) => typeof m === "function" && m.toString().includes("allowedTypes"));
		Cartify.URI.fromString = URIModules.find((m) => typeof m === "function" && m.toString().includes("Argument `uri`"));

		// createURI functions
		const createURIFunctions = URIModules.filter((m) => typeof m === "function" && m.toString().match(/\([\w$]+\./));
		for (const type of Object.keys(Cartify.URI.Type)) {
			...
			const camelCaseType = type
				...
				.join("");
			Cartify.URI[`${camelCaseType}URI`] = func;
		}
		...
		for (const type of Object.keys(Cartify.URI.Type)) {
			...
			Cartify.URI[`is${camelCaseType}`] =
				func ??
				((uri) => {
					let uriObj;
					try {
						uriObj = Cartify.URI.from?.(uri) ?? Cartify.URI.fromString?.(uri);
					} catch {
						return false;
					}
					if (!uriObj) return false;
					return uriObj.type === Cartify.URI.Type[type];
				});
		}

		Cartify.URI.isPlaylistV1OrV2 = (uri) => Cartify.URI.isPlaylist(uri) || Cartify.URI.isPlaylistV2(uri);

		// Conversion functions
		Cartify.URI.idToHex = URIModules.find((m) => typeof m === "function" && m.toString().includes("22==="));
		Cartify.URI.hexToId = URIModules.find((m) => typeof m === "function" && m.toString().includes("32==="));

		// isSameIdentity
		Cartify.URI.isSameIdentity = URIModules.find((m) => typeof m === "function" && m.toString().match(/[\w$]+\.id===[\w$]+\.id/));
	})();

	Cartify.Events.webpackLoaded.fire();
	refreshNavLinks?.();
})();
...
// Wait for Cartify.Player.origin._state before adding following APIs
(function waitOrigins() {
	if (!Cartify?.Player?.origin?._state) {
	...
	const interval = setInterval(() => {
		if (!Cartify.Player.origin._state?.item) return;
		Cartify.Player.data = Cartify.Player.origin._state;
		playerState.cache = Cartify.Player.data;
		clearInterval(interval);
	}, 10);

	Cartify.Player.origin._events.addListener("update", ({ data: playerEventData }) => {
		playerState.current = playerEventData.item ? playerEventData : null;
		Cartify.Player.data = playerState.current;

		if (playerState.cache?.item?.uri !== playerState.current?.item?.uri) {
			const event = new Event("songchange");
			event.data = Cartify.Player.data;
			Cartify.Player.dispatchEvent(event);
		}
	...
	(function waitProductStateAPI() {
		if (!Cartify.Platform?.UserAPI) {
			setTimeout(waitProductStateAPI, 100);
			return;
		}

		const productState = Cartify.Platform.UserAPI._product_state || Cartify.Platform.UserAPI._product_state_service;
		if (productState) return;
		if (!Cartify.Platform?.ProductStateAPI) {
			setTimeout(waitProductStateAPI, 100);
			return;
		}

		const productStateApi = Cartify.Platform.ProductStateAPI.productStateApi;
		Cartify.Platform.UserAPI._product_state_service = productStateApi;
	})();

	(async function setButtonsHeight() {
		while (!Cartify.CosmosAsync) {
			await new Promise((res) => setTimeout(res, 100));
		}
		const expFeatures = JSON.parse(localStorage.getItem("Cartify-exp-features") || "{}");
		const isGlobalNavbar = expFeatures?.enableGlobalNavBar?.value;

		if (typeof isGlobalNavbar !== "undefined" && isGlobalNavbar === "control") {
			await Cartify.CosmosAsync.post("sp://messages/v1/container/control", {
				type: "update_titlebar",
				height: Cartify.Platform.PlatformData.os_name === "osx" ? "42" : "40",
			});
		}
	})();
	...
	setInterval(() => {
		if (playerState.cache?.isPaused === false) {
			...
			event.data = Cartify.Player.getProgress();
			Cartify.Player.dispatchEvent(event);
		}
	}, 100);

	Cartify.addToQueue = (uri) => {
		return Cartify.Player.origin._queue.addToQueue(uri);
	};
	Cartify.removeFromQueue = (uri) => {
		return Cartify.Player.origin._queue.removeFromQueue(uri);
	};
})();

Cartify.getAudioData = async (uri) => {
	const providedURI = uri || Cartify.Player.data.item.uri;
	const uriObj = Cartify.URI.from?.(providedURI) ?? Cartify.URI.fromString?.(providedURI);
	if (!uriObj || (uriObj.Type || uriObj.type) !== Cartify.URI.Type.TRACK) {
		throw "URI is invalid.";
	}

	return await Cartify.CosmosAsync.get(
		`https://spclient.wg.spotify.com/audio-attributes/v1/audio-analysis/${uriObj.getBase62Id?.() ?? uriObj.id}?format=json`
	);
};

Cartify.colorExtractor = async (uri) => {
	const body = await Cartify.CosmosAsync.get(`https://spclient.wg.spotify.com/colorextractor/v1/extract-presets?uri=${uri}&format=json`);
...
Cartify._getStyledClassName = (args, component) => {
	const includedKeys = [
		"role",
		"variant",
		"semanticColor",
...
(function waitMouseTrap() {
	...
	function formatKeys(keys) {
		...
		if (typeof keys === "object") {
			if (!keys.key || !Object.values(KEYS).includes(keys.key)) {
				throw `Cartify.Keyboard.registerShortcut: Invalid key ${keys.key}`;
			}
			...
			throw `Cartify.Keyboard.registerShortcut: Invalid keys ${keys}`;
		}
		return keystroke;
	}
	...
	Cartify.Keyboard = {
		...
		registerShortcut: (keys, callback) => {
			Cartify.Mousetrap.bind(formatKeys(keys), callback);
		},
		_deregisterShortcut: (keys) => {
			Cartify.Mousetrap.unbind(formatKeys(keys));
		},
		changeShortcut: (keys, newKeys) => {
			if (!keys || !newKeys) throw "Cartify.Keyboard.changeShortcut: Invalid keys";

			const callback = Object.keys(Cartify.Mousetrap.trigger()._directMap).find((key) => key.startsWith(formatKeys(keys)));
			if (!callback) throw "Cartify.Keyboard.changeShortcut: Shortcut not found";

			Cartify.Keyboard.registerShortcut(newKeys, Cartify.Mousetrap.trigger()._directMap[callback]);
			Cartify.Keyboard._deregisterShortcut(keys);
		},
	};
	Cartify.Keyboard.registerIsolatedShortcut = Cartify.Keyboard.registerShortcut;
	Cartify.Keyboard.registerImportantShortcut = Cartify.Keyboard.registerShortcut;
	Cartify.Keyboard.deregisterImportantShortcut = Cartify.Keyboard._deregisterShortcut;
})();
...
(async function waitUserAPI() {
	...
	const productState =
		Cartify.Platform.UserAPI?._product_state ||
		Cartify.Platform.UserAPI?._product_state_service ||
		Cartify.Platform?.ProductStateAPI.productStateApi;
...
function parseIcon(icon, size = 16) {
	if (icon && Cartify.SVGIcons[icon]) {
		return `<svg height="${size}" width="${size}" viewBox="0 0 ${size} ${size}" fill="currentColor">${Cartify.SVGIcons[icon]}</svg>`;
	}
	return icon || "";
}

function createIconComponent(icon, size = 16) {
	return Cartify.React.createElement(
		Cartify.ReactComponent.IconComponent,
		{
			iconSize: size,
...
Cartify.ContextMenuV2 = (() => {
	...
	class Item {
		constructor({ children, disabled = false, leadingIcon, trailingIcon, divider, onClick, shouldAdd = () => true }) {
			...
			this._element = Cartify.ReactJSX.jsx(() => {
				const [_children, setChildren] = Cartify.React.useState(this._children);
				const [_disabled, setDisabled] = Cartify.React.useState(this._disabled);
				const [_leadingIcon, setLeadingIcon] = Cartify.React.useState(this._leadingIcon);
				const [_trailingIcon, setTrailingIcon] = Cartify.React.useState(this._trailingIcon);
				const [_divider, setDivider] = Cartify.React.useState(this._divider);

				Cartify.React.useEffect(() => {
				...
				const context = Cartify.React.useContext(Cartify.ContextMenuV2._context) ?? {};

				return Cartify.React.createElement(Cartify.ReactComponent.MenuItem, {
					disabled: _disabled,
					divider: _divider,
					onClick: (e) => {
		...
		register() {
			Cartify.ContextMenuV2.registerItem(this._element, this.shouldAdd);
		}
		deregister() {
			Cartify.ContextMenuV2.unregisterItem(this._element);
		}
	}
	...
	class ItemSubMenu {
		...
		constructor({ text, disabled = false, leadingIcon, divider, items, depth = 1, shouldAdd = () => true }) {
			...
			this._depth = depth;
			this._element = Cartify.ReactJSX.jsx(() => {
				const [_text, setText] = Cartify.React.useState(this._text);
				const [_disabled, setDisabled] = Cartify.React.useState(this._disabled);
				const [_leadingIcon, setLeadingIcon] = Cartify.React.useState(this._leadingIcon);
				const [_divider, setDivider] = Cartify.React.useState(this._divider);
				const [_items, setItems] = Cartify.React.useState(this._items);
				const [_depth, setDepth] = Cartify.React.useState(this._depth);
				...
				return Cartify.React.createElement(Cartify.ReactComponent.MenuSubMenuItem, {
					displayText: _text,
					divider: _divider,
					depth: _depth,
	...
	const renderItems = () => {
		const { props, trigger, target } = Cartify.React.useContext(Cartify.ContextMenuV2._context) ?? {};
...
Cartify.Menu = (() => {
	...
	class Item extends Cartify.ContextMenuV2.Item {
	...
	class SubMenu extends Cartify.ContextMenuV2.ItemSubMenu {
...
Cartify.ContextMenu = (() => {
	const iconList = Object.keys(Cartify.SVGIcons);
	...
	class Item extends Cartify.ContextMenuV2.Item {
		...
		constructor(name, onClick, shouldAdd = () => true, icon = undefined, trailingIcon = undefined, disabled = false) {
			super({
				...
				trailingIcon,
				onClick: (context) => {
					const [uris, uids, contextUri] = Cartify.ContextMenuV2.parseProps(context.props);
					onClick(uris, uids, contextUri);
				},
				shouldAdd: (props) => {
					const parsedProps = Cartify.ContextMenuV2.parseProps(props);
...
Cartify._renderNavLinks = (list, isTouchScreenUi) => {
	const [, refresh] = Cartify.React.useReducer((x) => x + 1, 0);
	refreshNavLinks = refresh;

	if (
		!Cartify.ReactComponent.ButtonTertiary ||
		!Cartify.ReactComponent.Navigation ||
		!Cartify.ReactComponent.TooltipWrapper ||
		!Cartify.ReactComponent.ScrollableContainer ||
		!Cartify.Platform.History ||
		!Cartify.Platform.LocalStorageAPI
	)
		return;

	const navLinkFactory = isTouchScreenUi ? NavLinkGlobal : NavLinkSidebar;

	if (!navLinkFactoryCtx) navLinkFactoryCtx = Cartify.React.createContext(null);
	const registered = [];
	...
	for (const app of list) {
		...
		try {
			...
			request.open("GET", `Cartify-routes-${app}.json`, false);
			request.send(null);
		...
		if (typeof appProper === "object") {
			appProper = appProper[Cartify.Locale?.getLocale()] || appProper.en;
		}
		if (!appProper) {
	...
	(function addStyling() {
		if (document.querySelector("style.Cartify-navlinks")) return;
		const style = document.createElement("style");
		style.className = "Cartify-navlinks";
		style.innerHTML = `
	:root {
	...
	const wrapScrollableContainer = (element) =>
		Cartify.React.createElement(
			...
			Cartify.React.createElement(Cartify.ReactComponent.ScrollableContainer, null, element)
		);
	...
	const NavLinks = () =>
		Cartify.React.createElement(
			...
			registered.map((NavLinkElement) => Cartify.React.createElement(NavLink, NavLinkElement, null))
		);
...
const NavLink = ({ appProper, appRoutePath, icon, activeIcon }) => {
	const isActive = Cartify.Platform.History.location.pathname?.startsWith(appRoutePath);
	const createIcon = () => createIconComponent(isActive ? activeIcon : icon, 24);

	const NavLinkFactory = Cartify.React.useContext(navLinkFactoryCtx);

	return NavLinkFactory && Cartify.React.createElement(NavLinkFactory, { appProper, appRoutePath, createIcon, isActive }, null);
};

const NavLinkSidebar = ({ appProper, appRoutePath, createIcon, isActive }) => {
	const isSidebarCollapsed = Cartify.Platform.LocalStorageAPI.getItem("ylx-sidebar-state") === 1;
		...
		Cartify.React.createElement(
			...
			Cartify.React.createElement(
				Cartify.ReactComponent.Navigation,
				{
					to: appRoutePath,
					referrer: "other",
					className: Cartify.classnames("link-subtle", "main-yourLibraryX-navLink", {
				...
				createIcon(),
				!isSidebarCollapsed && Cartify.React.createElement(Cartify.ReactComponent.TextComponent, { variant: "balladBold" }, appProper)
			)
		)
	);
};
...
const NavLinkGlobal = ({ appProper, appRoutePath, createIcon, isActive }) => {
	return Cartify.React.createElement(
		Cartify.ReactComponent.TooltipWrapper,
		{ label: appProper },
		Cartify.React.createElement(Cartify.ReactComponent.ButtonTertiary, {
			iconOnly: createIcon,
			className: Cartify.classnames("link-subtle", "main-globalNav-navLink", "main-globalNav-link-icon", "custom-navlink", {
				"main-globalNav-navLinkActive": isActive,
			}),
			"aria-label": appProper,
			onClick: () => Cartify.Platform.History.push(appRoutePath),
		})
	);
};
...
class _HTMLGenericModal extends HTMLElement {
	hide() {
		Cartify.ReactDOM.unmountComponentAtNode(this.querySelector("main"));
		this.remove();
	}
...
<div class="GenericModal__overlay" style="z-index: 100;">
	<div class="GenericModal" tabindex="-1" role="dialog" aria-label="${title}" aria-modal="true">
		...
		if (Cartify.React.isValidElement(content)) {
			Cartify.ReactDOM.render(content, main);
...
Cartify.PopupModal = new _HTMLGenericModal();

Object.defineProperty(Cartify, "TippyProps", {
	value: {
		delay: [200, 0],
		animation: true,
		render(instance) {
...
Cartify.Topbar = (() => {
	...
	class Button {
		constructor(label, icon, onClick, disabled = false, isRight = false) {
			...
			this.tippy = Cartify.Tippy?.(this.element, {
				content: label,
				...Cartify.TippyProps,
			});
			this.label = label;
		...
		set icon(input) {
			let newInput = input;
			if (newInput && Cartify.SVGIcons[newInput]) {
				newInput = `<svg height="16" width="16" viewBox="0 0 16 16" fill="currentColor">${Cartify.SVGIcons[newInput]}</svg>`;
			}
			this._icon = newInput;
	...
	(function waitForPlatform() {
		if (!Cartify.Platform?.History) {
			setTimeout(waitForPlatform, 100);
			return;
		}

		Cartify.Platform.History.listen(() => waitForTopbarMounted());
	})();
...
Cartify.Playbar = (() => {
	...
	class Button {
		...
		set icon(input) {
			let newInput = input;
			if (newInput && Cartify.SVGIcons[newInput]) {
				newInput = `<svg height="16" width="16" viewBox="0 0 16 16" fill="currentColor" stroke="currentColor">${Cartify.SVGIcons[newInput]}</svg>`;
			}
			this._icon = newInput;
...
(async function checkForUpdate() {
	...
	const { check_Cartify_update, version } = Cartify.Config;
	// Skip checking if upgrade check is disabled, or version is Dev/version is not set
	if (!check_Cartify_update || !version || version === "Dev") return;
	// Fetch latest version from GitHub
	try {
		let changelog;
		const res = await fetch("https://api.github.com/repos/crtwheel/cartify/releases/latest");
		...
		if (semver !== version) {
			...
			content.innerHTML = `
				<style>
					...
					#Cartify-update ol > li {
						list-style-type: decimal;
					}
					.Cartify-update-space {
						margin-bottom: 25px;
					}
					.Cartify-update-little-space {
						margin-bottom: 8px;
					}
				</style>
				<p class="Cartify-update-space">Update Cartify to receive new features and bug fixes.</p>
				...
				<ol>
					<li>Update Cartify CLI</li>
					<pre class="Cartify-update-little-space">Cartify update</pre>
					<p>Cartify will automatically apply changes to Spotify after upgrading to the latest version.</p>
					<p>If you installed Cartify via a package manager, update using said package manager.</p>
				</ol>
			`;
			...
			(function waitForTippy() {
				...
				const tippy = Cartify.Tippy(content.querySelectorAll("pre"), {
				...
				for (const instance of tippy) {
					instance.reference.addEventListener("click", () => {
						Cartify.Platform.ClipboardAPI.copy(instance.reference.textContent);
			...
			const updateModal = {
				title: "Update Cartify",
				content,
				isLarge: true,
			};

			new Cartify.Topbar.Button(
				"Update Cartify",
				...
				() => Cartify.PopupModal.display(updateModal)
			);
		}
	} catch (err) {
		console.error(err);
