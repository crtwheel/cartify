...
(function KeyboardShortcut() {
	...
	const binds = {
		...
		"ctrl+q": {
			callback: () =>
				Cartify.CosmosAsync.post("sp://esperanto/spotify.desktop.lifecycle_esperanto.proto.DesktopLifecycle/Shutdown") &&
				Cartify.CosmosAsync.post("sp://desktop/v1/shutdown"),
		},
		...
		"shift+h": { callback: () => Cartify.Platform.History.goBack() },
		"shift+l": { callback: () => Cartify.Platform.History.goForward() },

		// M to Like/Unlike track
		m: { callback: () => Cartify.Player.toggleHeart() },

		// Forward Slash to open search page
		"/": { callback: () => Cartify.Platform.History.replace("/search") },

		// CTRL + Arrow Left Next and CTRL + Arrow Right  Previous Song
		"ctrl+left": { callback: () => Cartify.Player.back() },
		"ctrl+right": { callback: () => Cartify.Player.next() },

		// CTRL + Arrow Up Increase Volume CTRL + Arrow Down Decrease Volume
		"ctrl+up": { callback: () => Cartify.Player.setVolume(Cartify.Player.getVolume() + 0.05) },
		"ctrl+down": { callback: () => Cartify.Player.setVolume(Cartify.Player.getVolume() - 0.05) },
	...
	for (const [key, { staticCondition, callback }] of Object.entries(binds)) {
		if (typeof staticCondition === "undefined" || staticCondition) {
			Cartify.Mousetrap.bind(key, (event) => {
	...
	function findActiveIndex(allItems) {
		...
		const historyURI = Cartify.Platform.History.location.pathname.replace(/^\//, "spotify:").replace(/\//g, ":");
	...
	function rotateSidebar(direction) {
		const allItems = document.querySelectorAll(
			"#Cartify-sticky-list .main-yourLibraryX-navLink, .main-yourLibraryX-listItem > div:not(:has([data-skip-in-keyboard-nav])) > div:first-child"
		);
...
.vim-key {
	...
	const mousetrap = new Cartify.Mousetrap(document);
	...
	/**
	 *
	 * @param {Cartify.Keyboard.ValidKey} key
	 */
	this.setCancelKey = function (key) {
		mousetrap.bind(Cartify.Keyboard.KEYS[key], this.deactivate.bind(this));
	};
