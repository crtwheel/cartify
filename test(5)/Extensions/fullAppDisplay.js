...
(function FullAppDisplay() {
	if (!Cartify.Keyboard || !Cartify.React || !Cartify.ReactDOM) {
		setTimeout(FullAppDisplay, 200);
		return;
	}

	const { React: react, ReactDOM: reactDOM } = Cartify;
...
#fad-art {
	...
	const ProgressBar = () => {
		const [progress, setProgress] = useState(Cartify.Player.getProgress());
		const duration = Cartify.Platform.PlayerAPI._state.duration;
		...
		useEffect(() => {
			...
			Cartify.Player.addEventListener("onprogress", update);
			return () => Cartify.Player.removeEventListener("onprogress", update);
		}, [isDragging]);
		...
		const handleClick = (e) => {
			...
			Cartify.Player.seek(newProgress);
		...
		return react.createElement(
			...
			react.createElement("span", { id: "fad-elapsed" }, Cartify.Player.formatTime(progress)),
			react.createElement(
				"div",
				{
					id: "fad-progress",
			...
			react.createElement("span", { id: "fad-duration" }, Cartify.Player.formatTime(duration))
		);
	};

	const PlayerControls = () => {
		const [value, setValue] = useState(Cartify.Player.isPlaying());
		useEffect(() => {
			const update = ({ data }) => setValue(!data.isPaused);
			Cartify.Player.addEventListener("onplaypause", update);
			return () => Cartify.Player.removeEventListener("onplaypause", update);
		});
		return react.createElement(
			"div",
			{ id: "fad-controls" },
			react.createElement(ButtonIcon, {
				icon: Cartify.SVGIcons["skip-back"],
				onClick: Cartify.Player.back,
			}),
			react.createElement(ButtonIcon, {
				icon: Cartify.SVGIcons[value ? "pause" : "play"],
				onClick: Cartify.Player.togglePlay,
			}),
			react.createElement(ButtonIcon, {
				icon: Cartify.SVGIcons["skip-forward"],
				onClick: Cartify.Player.next,
			})
		);
	};
	...
	class FAD extends react.Component {
		constructor(props) {
			...
			this.state = {
				...
				releaseDate: "",
				cover: "",
				heart: Cartify.Player.getHeart(),
			};
			this.currTrackImg = new Image();
			this.nextTrackImg = new Image();
			this.mousetrap = new Cartify.Mousetrap();
		}

		async getAlbumDate(uri) {
			const { getAlbum } = Cartify.GraphQL.Definitions;
			const { errors, data } = await Cartify.GraphQL.Request(getAlbum, {
				uri,
		...
		async fetchInfo() {
			const meta = Cartify.Player.data.item.metadata;
		...
		componentDidMount() {
			...
			Cartify.Player.addEventListener("songchange", this.updateInfo);
			this.updateInfo();
			...
			Cartify.Platform.PlayerAPI._events.addListener("queue_update", this.onQueueChange);
			this.mousetrap.bind("esc", deactivate);
			window.dispatchEvent(new Event("fad-request"));
		}

		componentWillUnmount() {
			Cartify.Player.removeEventListener("songchange", this.updateInfo);
			Cartify.Platform.PlayerAPI._events.removeListener("queue_update", this.onQueueChange);
					...
					react.createElement(
								...
								react.createElement(
									...
									react.createElement(
										...
										react.createElement(DisplayIcon, {
											icon: Cartify.SVGIcons[this.state.heart ? "heart-active" : "heart"],
											size: 50,
										})
									)
								),
						...
						react.createElement(
							...
							react.createElement(SubInfo, {
								...
								icon: Cartify.SVGIcons.artist,
							}),
							CONFIG.showAlbum &&
								react.createElement(SubInfo, {
									id: "fad-album",
									text: this.state.album,
									icon: Cartify.SVGIcons.album,
								}),
							CONFIG.showReleaseDate &&
								react.createElement(SubInfo, {
									id: "fad-release-date",
									text: this.state.releaseDate,
									icon: Cartify.SVGIcons.clock,
								}),
							react.createElement(
								"div",
								{
	...
	function deactivate() {
		...
		if (lastApp && lastApp !== "/lyrics-plus") {
			Cartify.Platform.History.push(lastApp);
		}
	}
	...
	function checkLyricsPlus() {
		return Cartify.Config?.custom_apps?.includes("lyrics-plus") || !!document.querySelector("a[href='/lyrics-plus']");
	}

	function requestLyricsPlus() {
		if (CONFIG.lyricsPlus && checkLyricsPlus()) {
			lastApp = Cartify.Platform.History.location.pathname;
			if (lastApp !== "/lyrics-plus") {
				Cartify.Platform.History.push("/lyrics-plus");
			}
		}
		window.dispatchEvent(new Event("fad-request"));
	}
	...
	function getConfig() {
		try {
			const parsed = JSON.parse(Cartify.LocalStorage.get("full-app-display-config") || "{}");
			if (parsed && typeof parsed === "object") {
				return parsed;
			}
			throw "";
		} catch {
			Cartify.LocalStorage.set("full-app-display-config", "{}");
			return {};
		}
	}

	function saveConfig() {
		Cartify.LocalStorage.set("full-app-display-config", JSON.stringify(CONFIG));
	}
...
`,
	...
	new Cartify.Topbar.Button(
		"Full App Display",
		`<svg role="img" height="16" width="16" viewBox="0 0 16 16" fill="currentColor">${Cartify.SVGIcons.projector}</svg>`,
		activate
	);

	Cartify.Mousetrap.bind("f11", toggleFad);
})();
