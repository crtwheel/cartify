...
(function Bookmark() {
	const { CosmosAsync, Player, LocalStorage, ContextMenu, URI } = Cartify;
	if (!(CosmosAsync && URI)) {
	...
	// Local Storage keys
	const STORAGE_KEY = "bookmark_Cartify";
...
`;
			...
			Cartify.Tippy(this.querySelectorAll("[data-tippy-content]"), Cartify.TippyProps);
			if (isPlayable) {
	...
	function createMenuItem(title, callback) {
		...
		Cartify.ReactDOM.render(
			Cartify.React.createElement(
				Cartify.ReactComponent.MenuItem,
				{
					onClick: () => {
						callback?.();
					},
	...
	async function storeThisPage() {
		...
		const context = Cartify.Platform.History.location.pathname;
		try {
			contextUri = Cartify.URI.fromString(context);
		} catch (e) {
			Cartify.showNotification("Cannot bookmark this page", true);
			return;
		}
	...
	function getTrackMeta() {
		...
		const playerState = Cartify.Player.data;
...
`;
	...
	async function onLinkClick(info) {
		if (info.context?.startsWith("/")) {
			Cartify.Platform.History.push(info.context);
			return;
		}
		const url = Cartify.URI.fromString(info.uri).toURLPath(true);
		Cartify.Platform.History.push(url);
	}
	...
	function onPlayClick(info) {
		...
		Cartify.Player.playUri(uri, {}, options);
	}

	const fetchAlbum = async (uri) => {
		const { getAlbum } = Cartify.GraphQL.Definitions;
		const { data } = await Cartify.GraphQL.Request(getAlbum, {
			uri,
	...
	const fetchArtist = async (uri) => {
		...
		const { data } = await Cartify.GraphQL.Request(queryArtistOverview, {
			uri,
			locale: Cartify.Locale.getLocale(),
	...
	const fetchTrack = async (uri, uid, context) => {
		...
		let newContext;
		if (context && uid && Cartify.URI.isPlaylistV1OrV2(context)) {
			newContext = `${Cartify.URI.fromString(context).toURLPath(true)}?uid=${uid}`;
		}
		return {
			uri,
			title: res.name,
	...
	const fetchPlaylist = async (uri) => {
		const res = await Cartify.CosmosAsync.get(`sp://core-playlist/v1/playlist/${uri}/metadata`, {
	...
	new Cartify.ContextMenu.Item(
		...
		async ([uri], [uid] = [], context = undefined) => {
			...
			let meta;
			switch (type) {
				case Cartify.URI.Type.TRACK:
					meta = await fetchTrack(uri, uid, context);
					break;
				case Cartify.URI.Type.ALBUM:
					meta = await fetchAlbum(uri);
					break;
				case Cartify.URI.Type.ARTIST:
					meta = await fetchArtist(uri);
					break;
				case Cartify.URI.Type.SHOW:
					meta = await fetchShow(uri);
					break;
				case Cartify.URI.Type.EPISODE:
					meta = await fetchEpisode(uri);
					break;
				case Cartify.URI.Type.PLAYLIST:
				case Cartify.URI.Type.PLAYLIST_V2:
