...
function PopupLyrics() {
	const { Player, CosmosAsync, LocalStorage, ContextMenu } = Cartify;
	...
	const LyricProviders = {
		...
		async fetchLrclib(info) {
			...
			const body = await fetch(finalURL, {
				headers: {
					"x-user-agent": `Cartify v${Cartify.Config.version} (https://github.com/crtwheel/cartify)`,
				},
			});
	...
	const button = new Cartify.Topbar.Button("Popup Lyrics", "lyrics", () => {
		if (!lyricVideoIsOpen) {
	...
	async function updateTrack(refresh = false) {
		...
		if (!Cartify.URI.isTrack(Player.data.item.uri) && !Cartify.URI.isLocalTrack(Player.data.item.uri)) {
			return;
		}
...
<div class="setting-row">
	...
	function musixmatchTokenElements(defaultVal, id) {
		...
		function clickRefresh() {
			...
			Cartify.CosmosAsync.get("https://apic-desktop.musixmatch.com/ws/1.1/token.get?app_id=web-desktop-app-v1.0", null, {
				authority: "apic-desktop.musixmatch.com",
			})
    ...
    <div class="col action">
        <button class="switch small">
            <svg height="10" width="10" viewBox="0 0 16 16" fill="currentColor">
                ${Cartify.SVGIcons["chart-up"]}
            </svg>
        </button>
        <button class="switch small">
            <svg height="10" width="10" viewBox="0 0 16 16" fill="currentColor">
                ${Cartify.SVGIcons["chart-down"]}
            </svg>
        </button>
        <button class="switch">
            <svg height="16" width="16" viewBox="0 0 16 16" fill="currentColor">
