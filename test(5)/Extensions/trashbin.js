...
(function TrashBin() {
	...
	if (!Cartify.Player.data || !Cartify.LocalStorage || !skipBackBtn) {
	...
	function createSlider(name, desc, defaultVal, callback) {
		...
		slider.onclick = () => {
			...
			Cartify.LocalStorage.set(name, state);
	...
	function settingsContent() {
		...
		content.appendChild(
			createButton("Clear ", "Clear all items from trashbin (cannot be reverted).", () => {
				...
				putDataLocal();
				Cartify.showNotification("Trashbin cleared!");
			})
		);
	}
	...
	function initValue(item, defaultValue) {
		try {
			const value = JSON.parse(Cartify.LocalStorage.get(item));
	...
	const widget = new Cartify.Playbar.Widget(
		THROW_TEXT,
		trashbinIcon,
		(self) => {
			const uri = Cartify.Player.data.item.uri;
			const uriObj = Cartify.URI.fromString(uri);
			const type = uriObj.type;

			if (!trashSongList[uri]) {
				trashSongList[uri] = true;
				if (shouldSkipCurrentTrack(uri, type)) Cartify.Player.next();
				Cartify.showNotification("Song added to trashbin");
			} else {
				delete trashSongList[uri];
				setWidgetState(false);
				Cartify.showNotification("Song removed from trashbin");
			}
	...
	setWidgetState(
		trashSongList[Cartify.Player.data.item.uri],
		Cartify.URI.fromString(Cartify.Player.data.item.uri).type !== Cartify.URI.Type.TRACK
	);
	...
	function refreshEventListeners(state) {
		...
		if (state) {
			...
			Cartify.Player.addEventListener("songchange", watchChange);
			enableWidget && widget.register();
			watchChange();
		} else {
			skipBackBtn.removeEventListener("click", eventListener);
			Cartify.Player.removeEventListener("songchange", watchChange);
			widget.deregister();
		}
	}
	...
	function watchChange() {
		...
		setWidgetState(isBanned, Cartify.URI.fromString(data.item.uri).type !== Cartify.URI.Type.TRACK);
	...
	function shouldSkipCurrentTrack(uri, type) {
		const curTrack = Cartify.Player.data.item;
		if (type === Cartify.URI.Type.TRACK) {
			if (uri === curTrack.uri) {
				return true;
			}
		}

		if (type === Cartify.URI.Type.ARTIST) {
			let count = 1;
	...
	function toggleThrow(uris) {
		...
		const list = type === Cartify.URI.Type.TRACK ? trashSongList : trashArtistList;

		if (!list[uri]) {
			list[uri] = true;
			if (shouldSkipCurrentTrack(uri, type)) Cartify.Player.next();
			Cartify.Player.data?.item.uri === uri && setWidgetState(true);
			Cartify.showNotification(type === Cartify.URI.Type.TRACK ? "Song added to trashbin" : "Artist added to trashbin");
		} else {
			delete list[uri];
			Cartify.Player.data?.item.uri === uri && setWidgetState(false);
			Cartify.showNotification(type === Cartify.URI.Type.TRACK ? "Song removed from trashbin" : "Artist removed from trashbin");
		}
	...
	function shouldAddContextMenu(uris) {
		...
		const uri = uris[0];
		const uriObj = Cartify.URI.fromString(uri);
		if (uriObj.type === Cartify.URI.Type.TRACK) {
			cntxMenu.name = trashSongList[uri] ? UNTHROW_TEXT : THROW_TEXT;
			return true;
		}

		if (uriObj.type === Cartify.URI.Type.ARTIST) {
	...
	const cntxMenu = new Cartify.ContextMenu.Item(THROW_TEXT, toggleThrow, shouldAddContextMenu, trashbinIcon);
	cntxMenu.register();

	function putDataLocal() {
		Cartify.LocalStorage.set("TrashSongList", JSON.stringify(trashSongList));
		Cartify.LocalStorage.set("TrashArtistList", JSON.stringify(trashArtistList));
	}
	...
	function copyItems() {
		...
		Cartify.Platform.ClipboardAPI.copy(JSON.stringify(data));
		Cartify.showNotification("Copied to clipboard");
	}
	...
	async function exportItems() {
		...
		try {
			const handle = await window.showSaveFilePicker({
				suggestedName: "Cartify-trashbin.json",
				types: [
					{
						description: "Cartify trashbin backup",
						accept: {
			...
			Cartify.showNotification("Backup saved succesfully.");
		} catch {
			Cartify.showNotification("Failed to save, try copying trashbin contents to clipboard and creating a backup manually.");
		}
	}
		...
		input.onchange = (e) => {
			...
			reader.onload = (e) => {
				try {
					...
					putDataLocal();
					Cartify.showNotification("File Import Successful!");
				} catch (e) {
					Cartify.showNotification("File Import Failed!", true);
					console.error(e);
				}
			};
			reader.onerror = () => {
				Cartify.showNotification("File Read Failed!", true);
