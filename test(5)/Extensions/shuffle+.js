...
(async function shufflePlus() {
	if (!(Cartify.CosmosAsync && Cartify.Platform)) {
	...
	function getConfig() {
		try {
			const parsed = JSON.parse(Cartify.LocalStorage.get("shufflePlus:settings"));
			if (parsed && typeof parsed === "object") {
				return parsed;
			}
			throw "";
		} catch {
			Cartify.LocalStorage.set("shufflePlus:settings", "{}");
			return {
				artistMode: "all",
	...
	function saveConfig() {
		Cartify.LocalStorage.set("shufflePlus:settings", JSON.stringify(CONFIG));
	}
	...
	new Cartify.Menu.Item("Shuffle+", false, settingsPage, "shuffle").register();

	const { Type } = Cartify.URI;

	function shouldAddShufflePlus(uri) {
		if (uri.length === 1) {
			const uriObj = Cartify.URI.fromString(uri[0]);
			switch (uriObj.type) {
				case Type.PLAYLIST:
	...
	function shouldAddShufflePlusLiked(uri) {
		const uriObj = Cartify.URI.fromString(uri[0]);
		if (Cartify.Platform.History.location.pathname === "/collection/tracks") {
			switch (uriObj.type) {
				case Type.TRACK:
					return true;
			}
	...
	function shouldAddShufflePlusLocal(uri) {
		const uriObj = Cartify.URI.fromString(uri[0]);
		if (Cartify.Platform.History.location.pathname === "/collection/local-files") {
			switch (uriObj.type) {
				case Type.TRACK:
	...
	function renderQueuePlaybarButton() {
		if (!playbarButton) {
			playbarButton = new Cartify.Playbar.Button(
	...
	async function fetchPlaylistTracks(uri) {
		const res = await Cartify.Platform.PlaylistAPI.getContents(`spotify:playlist:${uri}`, {
			limit: 9999999,
		});
	...
	async function fetchFolderTracks(uri) {
		const res = await Cartify.Platform.RootlistAPI.getContents();
		...
		async function fetchNested(folder) {
			...
			for (const i of folder.items) {
				if (i.type === "playlist") {
					const uriObj = Cartify.URI.fromString(i.uri);
	...
	async function fetchAlbumTracks(uri, includeMetadata = false) {
		const { queryAlbumTracks } = Cartify.GraphQL.Definitions;
		const { data, errors } = await Cartify.GraphQL.Request(queryAlbumTracks, {
			uri,
			offset: 0,
			limit: 100,
		});
	...
	async function scanForTracksFromAlbums(res, artistName, type) {
		...
		for (const album of res) {
			...
			Cartify.showNotification(`${artistFetchTypeCount[type]} / ${res.length} ${type}s`);
	...
	async function fetchArtistTracks(uri) {
		...
		const discography = await Cartify.GraphQL.Request(queryArtistDiscographyAll, {
			uri,
			offset: 0,
		...
		const overview = await Cartify.GraphQL.Request(queryArtistOverview, {
			uri,
			locale: Cartify.Locale.getLocale(),
	...
	async function fetchArtistLikedTracks(uri) {
		const artistRes = await Cartify.CosmosAsync.get(`sp://core-collection/unstable/@/list/tracks/artist/${uri}?responseFormat=protobufJson`);
	...
	async function fetchArtistTopTenTracks(uri) {
		const { queryArtistOverview } = Cartify.GraphQL.Definitions;
		const { data, errors } = await Cartify.GraphQL.Request(queryArtistOverview, {
			uri,
			locale: Cartify.Locale.getLocale(),
			includePrerelease: false,
		});
	...
	async function fetchLikedTracks() {
		const res = await Cartify.CosmosAsync.get("sp://core-collection/unstable/@/list/tracks/all?responseFormat=protobufJson");
	...
	async function fetchLocalTracks() {
		const res = await Cartify.Platform.LocalFilesAPI.getTracks();

		return res.map((track) => track.uri);
	}

	function fetchQueue() {
		const { _queueState } = Cartify.Platform.PlayerAPI._queue;
	...
	async function fetchCollection(uriObj) {
		...
		const { pathname } = Cartify.Platform.History.location;
	...
	async function fetchShows(uri) {
		const res = await Cartify.CosmosAsync.get(`sp://core-show/v1/shows/${uri}?responseFormat=protobufJson`);
	...
	async function Queue(list, context, type) {
		...
		const { _queue, _client } = Cartify.Platform.PlayerAPI._queue;
		...
		if (context) {
			const { sessionId } = Cartify.Platform.PlayerAPI.getState();
			Cartify.Platform.PlayerAPI.updateContext(sessionId, {
				uri: context,
				url: `context://${context}`,
			});
		}
		...
		switch (type) {
			case Type.ARTIST:
				if (CONFIG.artistMode === "topTen") {
					Cartify.showNotification(`Shuffled Top ${count} Songs`);
					break;
				}
				if (CONFIG.artistMode === "likedSongArtist") {
					Cartify.showNotification(`Shuffled ${count} Liked Songs`);
					break;
				}
				if (CONFIG.artistMode === "single") {
					Cartify.showNotification(`Shuffled ${artistFetchTypeCount.single} Singles, Total of ${count} Songs`);
					break;
				}
				if (CONFIG.artistMode === "album") {
					Cartify.showNotification(`Shuffled ${artistFetchTypeCount.album} Albums, Total of ${count} Songs`);
					break;
				}
				Cartify.showNotification(`Shuffled ${artistFetchTypeCount.album} Albums, ${artistFetchTypeCount.single} Singles, Total of ${count} Songs`);
				break;
			default:
				Cartify.showNotification(`Shuffled ${count} Songs`);
		}
	...
	async function fetchAndPlay(rawUri) {
		...
		try {
			if (rawUri === "queue") {
				...
				const uriObj = Cartify.URI.fromString(rawUri);
				type = uriObj.type;
				...
				if (!list?.length) {
					Cartify.showNotification("Nothing to play", true);
					return;
				}
			...
			Cartify.showNotification(String(error), true);
			console.error(error);
		}
	}
})();
