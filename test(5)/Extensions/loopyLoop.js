...
`;
	...
	function saveState() {
		...
		if (!uri) return;
		Cartify.LocalStorage.set(`loopyLoop:${uri}`, JSON.stringify({ start, end, skipZones }));
	}
	...
	function loadState() {
		...
		if (!uri) return;
		try {
			const saved = Cartify.LocalStorage.get(`loopyLoop:${uri}`);
			if (saved) {
	...
	function applyMoveAdjustment(deltaSeconds) {
		const durationMs = Cartify.Player.getDuration();
	...
	Cartify.Player.addEventListener("onprogress", (event) => {
		...
		const durationMs = Cartify.Player.getDuration() || 0;
		...
		if (prevProgressPercent > threeSecFrac && percent < nearZeroFrac) {
			if (prevPressedAt > 0 && ts - prevPressedAt < 1500) {
				...
				prevPressedAt = ts;
				prevProgressPercent = percent;
				if (start !== null) Cartify.Player.seek(start);
				return;
			}
		}
		prevProgressPercent = percent;
		...
		if (end !== null && percent >= end) {
			// Cartify.Player.getRepeat(): 0 = off, 1 = repeat context, 2 = repeat track
			if (Cartify.Player.getRepeat() === 2) {
				if (ts - lastEndLoopSeek > 500) {
					lastEndLoopSeek = ts;
					Cartify.Player.seek(start ?? 0);
				}
			} else if (ts - lastNextCall > 2000) {
				lastNextCall = ts;
				seekStartPendingUri = Cartify.Player.data?.item?.uri ?? null;
				Cartify.Player.next();
			}
			return;
		}
	...
	Cartify.Player.addEventListener("songchange", () => {
		...
		if (Cartify.Player.data?.item?.uri !== seekStartPendingUri) seekStartPendingUri = null;
		loadState();
		drawOnBar();
		drawSkipMarkers();
	...
	const startBtn = createMenuItem("Set song start", () => {
		if (end !== null && mouseOnBarPercent >= end) {
			Cartify.showNotification("Song start must be before song end");
			return;
		}
	...
	const endBtn = createMenuItem("Set song end", () => {
		if (start !== null && mouseOnBarPercent <= start) {
			Cartify.showNotification("Song end must be after song start");
			return;
		}
	...
	const skipEndBtn = createMenuItem("Set section skip end", () => {
		if (pendingSkipStart === null) {
			Cartify.showNotification("No section skip start selected!");
			return;
		}
		...
		if (e > s) {
			if (skipZones.length < 10) {
				...
				Cartify.showNotification("Maximum 10 skip zones reached");
			}
		}
		pendingSkipStart = null;
	});
	...
	function tryLoadInitialState(attemptsLeft) {
		if (Cartify.Player.data?.item?.uri) {
			loadState();
			drawOnBar();
			drawSkipMarkers();
	...
	try {
		...
		const toolbarBtn = new Cartify.Playbar.Button("Loopy Loop", markerIcon, () => {
			mouseOnBarPercent = Cartify.Player.getProgressPercent();
