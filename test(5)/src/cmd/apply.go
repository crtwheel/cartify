...
import (
	...
	"github.com/crtwheel/cartify/src/apply"
...
// Apply .
func Apply(CartifyVersion string) {
	...
	backupCartifyVersion := backupSection.Key("with").MustString("")
	if CartifyVersion != backupCartifyVersion {
		utils.PrintInfo(`Preprocessed Spotify data is outdated. Please run "cartify restore backup apply" to receive new features and bug fixes`)
		os.Exit(1)
	}
	...
	if preprocSection.Key("expose_apis").MustBool(false) {
		utils.CopyFile(
			filepath.Join(utils.GetJsHelperDir(), "cartifyWrapper.js"),
			filepath.Join(appDestPath, "xpui", "helper"))
	}

	extensionList := featureSection.Key("extensions").Strings("|")
	if !slices.Contains(extensionList, "blockAds.js") {
		extensionList = append([]string{"blockAds.js"}, extensionList...)
	}
	...
	apply.AdditionalOptions(appDestPath, apply.Flag{
		...
		CheckCartifyUpdate: settingSection.Key("check_cartify_update").MustBool(false),
		...
		CartifyVer:         backupSection.Key("with").MustString(""),
	})
...
func refreshThemeCSS() {
	...
	var configJson CartifyConfigJson
	...
	if err != nil {
		...
		if err := os.WriteFile(
			filepath.Join(appDestPath, "xpui", "cartify-config.json"),
...
func CheckStates() {
	...
	if backStat.IsEmpty() {
		if spotStat.IsBackupable() {
			utils.PrintError(`You haven't backed up. Run "cartify backup apply"`)
		} else {
			utils.PrintError(`You haven't backed up and Spotify cannot be backed up at this state. Please re-install Spotify then run "cartify backup apply"`)
		}
		os.Exit(1)
		...
		if spotStat.IsMixed() {
			...
			utils.PrintInfo(`Please run "cartify backup apply"`)
		} else if spotStat.IsStock() {
			utils.PrintInfo(`Spotify client is in stock state`)
			utils.PrintInfo(`Please run "cartify backup apply"`)
		} else {
			utils.PrintInfo(`Spotify cannot be backed up at this state. Please re-install Spotify then run "cartify backup apply"`)
		}
