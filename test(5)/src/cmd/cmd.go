...
var (
	cartifyFolder         = utils.GetCartifyFolder()
	rawFolder, themedFolder = getExtractFolder()
	backupFolder            = utils.GetStateFolder("Backup")
	userThemesFolder        = utils.GetSubFolder(cartifyFolder, "Themes")
	quiet                   bool
	isAppX                  = false
	spotifyPath             string
	prefsPath               string
...
// tries to auto-detect them and stops Cartify when any one
// of them is invalid.
func InitPaths() {
	...
	if isAppX {
		appDestPath = filepath.Join(cartifyFolder, "AppX")
	} else {
		appDestPath = appPath
	}
...
func GetConfigPath() string {
	return filepath.Join(cartifyFolder, "config-xpui.ini")
}
...
func CheckUpdate(version string) {
	if !settingSection.Key("check_cartify_update").MustBool() || version == "Dev" {
		return
	}
	...
	if latestTag != version {
		...
		utils.PrintInfo(`Run "cartify update" or use a package manager to update Cartify`)
	}
}
