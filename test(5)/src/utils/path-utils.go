...
func MigrateConfigFolder() {
	if runtime.GOOS == "windows" {
		source := filepath.Join(os.Getenv("USERPROFILE"), ".Cartify")
		if _, err := os.Stat(source); err == nil {
			spinner, _ := Spinner.Start("Migrating config folder")
			destination := GetCartifyFolder()
...
func MigrateFolders() {
	backupPath := filepath.Join(GetCartifyFolder(), "Backup")
	extractedPath := filepath.Join(GetCartifyFolder(), "Extracted")
...
func GetCartifyFolder() string {
	result, isAvailable := os.LookupEnv("CARTIFY_CONFIG")
	...
	if runtime.GOOS == "windows" {
		...
		result = filepath.Join(parent, "cartify")
...
func GetStateFolder(name string) string {
	result, isAvailable := os.LookupEnv("CARTIFY_STATE")
	defer func() { CheckExistAndCreate(result) }()
...
var userAppsFolder = GetSubFolder(GetCartifyFolder(), "CustomApps")
var userExtensionsFolder = GetSubFolder(GetCartifyFolder(), "Extensions")
